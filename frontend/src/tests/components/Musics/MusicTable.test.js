...
import MusicTable, { showCell } from "main/components/Musics/MusicTable";
...
describe("MusicTable tests", () => {
  ...
  test("Has the expected column headers, content and buttons", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <MusicTable musics={musicFixtures.threeMusics} />
        </MemoryRouter>
      </QueryClientProvider>
    );
  ...
  test("Has the expected column headers, content and no buttons when showButtons=false", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <MusicTable musics={musicFixtures.threeMusics} showButtons={false} />
        </MemoryRouter>
      </QueryClientProvider>
    );
  ...
  test("Edit button navigates to the edit page", async () => {
    ...
    const expectedMessage = `editCallback: {"id":2,"title":"Cupid","album":"The Beginning","artist":"FIFTY FIFTY","genre":"K-pop"})`;
  ...
  test("Details button navigates to the details page", async () => {
    ...
    expect(console.log).toHaveBeenCalled();
    const message = console.log.mock.calls[0][0];
    const expectedMessage = `detailsCallback: {"id":2,"title":"Cupid","album":"The Beginning","artist":"FIFTY FIFTY","genre":"K-pop"})`;
    expect(message).toMatch(expectedMessage);
    restoreConsole();
  });
  ...
  test("Delete button calls delete callback", async () => {
    ...
    fireEvent.click(deleteButton);
     ...
     const message = console.log.mock.calls[0][0];
     const expectedMessage = `deleteCallback: {"id":2,"title":"Cupid","album":"The Beginning","artist":"FIFTY FIFTY","genre":"K-pop"})`;
     expect(message).toMatch(expectedMessage);
     restoreConsole();
  });
});
